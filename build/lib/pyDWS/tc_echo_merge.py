...
def tc_echo_merge(tc_icf, echo_icf, overlapN, overlap_threshold, fitP=False, savePlots=None, saveData=None):   
    ...
    if savePlots != None:
        plt.savefig(os.path.join(savePlots, 'twoCell_Interpolation.png'), transparent=True)
    ...
    plot_layout(fig, ax, True, False, 'Lag time t (s)', 'ICF')
    if savePlots != None:
        plt.savefig(os.path.join(savePlots, 'echo_Overlap.png'), transparent=True)
    ...
    if savePlots != None:
        plt.savefig(os.path.join(savePlots, 'second_Decay_fit.png'), transparent=True)
    ...
    ax.set_ylim(0, 1)
    if savePlots != None:
        plt.savefig(os.path.join(savePlots, 'tc_echo_corrected.png'), transparent=True)
    ...
    plot_layout(fig, ax, True, False, 'Lag time t (s)', r'ICF')
    if savePlots != None:
        plt.savefig(os.path.join(savePlots, 'tc_echo_merged.png'), transparent=True)
